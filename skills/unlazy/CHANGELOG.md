...
## 2.0.1 (local fixes, unreleased)

Upstream as PRs #7, #8 and #9.

- `gate-check.mjs`: a file argument given without `--timeout` was dropped (`i !== tIdx + 1` with `tIdx === -1`), so naming one gate file silently ran and rewrote every other one.
- `gate-check.mjs`: `CHECK:` commands are printed, not executed, until approved once with `--approve`. A `GATES.md` from a third party was arbitrary shell execution. Approval records live under `~/.unlazy/approved`, outside the directory being checked (a hostile checkout could otherwise ship its own approval), one file per gate file and command set so parallel leaves cannot clobber each other.
- `install-hooks.mjs`: identification required the string "unlazy" in the script path, so installing from a differently named directory produced duplicate Stop hooks and made `--uninstall` a no-op. The marker now travels in the command as an ignored argv tag (`--unlazy`, not a `#` comment, which `cmd.exe` does not understand), with a path fallback that removes entries written before it existed.
- `install-hooks.mjs`: an entry whose marker matches but whose script path has moved is stale, not current. It is replaced instead of reported as "Already installed", which used to leave settings pointing at a missing script with enforcement silently off.
